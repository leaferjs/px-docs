# PxGrow 更新日志

## 2026 / 07 / 17

> **v1.1.9**

### [Ruler 插件](/plugin/official/ruler/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

### [Video 插件](/plugin/official/video/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

## 2026 / 07 / 14

> **v1.1.8**

### [Linker 插件](/plugin/official/linker/index.md)

> v1.0.1

#### 🪲 修复

\- 🌸 连线元素被多层包裹后，拖动自动连线方向的元素，会导致连线偶发不渲染的问题

#### 🌷 感谢反馈

@black

### [Linker Editor 插件](/plugin/official/linker-editor/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

### [Mask Editor 插件](/plugin/official/mask-editor/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

### [Motion Editor 插件](/plugin/official/motion-editor/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

## 2026 / 07 / 10

> **v1.1.7**

### [Motion Text 插件](/plugin/official/motion-text/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

## 2026 / 07 / 07

> **v1.1.6**

### [Points Editor 插件](/plugin/official/points-editor/index.md)

> v1.0.0-beta

#### 🌱 上线首个测试版本

## 2026 / 06 / 30

> **v1.1.5**

### [Shape Editor 插件](/plugin/official/shape-editor/index.md)

> 1.0.0-beta

依赖于 LeaferJS [ v2.1.9](https://www.leaferjs.com/ui/update/#_2026-06-30)

#### 🌱 新增

\- 🌸 新增 Rect 元素编辑工具，可编辑圆角大小

#### 🪲 修复

\- 🌸 Ellipse 编辑工具控制点支持设置 locked 属性，控制是否允许拖动

#### 🌷 感谢反馈

@闰土

### [Path Editor 插件](/plugin/official/path-editor/index.md)

> v1.1.1

###### 🌱 新增

\- 🌸 首尾节点创建时拉出手柄不进行对称控制

#### 🌷 感谢反馈

@小志

## 2026 / 06 / 23

> **v1.1.4**

### [Clipper 插件](/plugin/official/clipper/index.md)

> v1.1.0

#### 🪲 修复

\- 🌸 限制拖拽范围时，移动裁剪框出区域回弹后，裁剪内容未更新的问题

#### 🌷 感谢反馈

@风

## 2026 / 06 / 16

> **v1.1.3**

### [Gradient Editor 插件](/plugin/official/gradient-editor/index.md)

> v1.0.1

#### 🪲 修复

\- 🌸 特殊路径元素下渐变编辑器位置会偏移的问题

#### 🌷 感谢反馈

@小志

### [Image Lighter 插件](/plugin/official/image-lighter/index.md)

> v1.1.3

###### 🪲 修复

\- 🌸 优化缩放比较小时，纹理可能会模糊的问题

### [Shape Editor 插件](/plugin/official/shape-editor/index.md)

> v1.0.0-alpha

#### 🌱 上线首个内测版本

## 2026 / 06 / 12

> **v1.1.2**

<!-- ### [Stroke Brush 插件](/plugin/official/stroke-brush/index.md)

> v1.0.0

#### 🌱 上线首个正式版本 -->

### [Image Lighter 插件](/plugin/official/image-lighter/index.md)

> v1.1.2

#### 🌱 新增

\- 🌸 支持 brush 纹理透明度扩展

## 2026 / 06 / 02

> **v1.1.1**

### [Image Lighter 插件](/plugin/official/image-lighter/index.md)

> v1.1.1

#### 🌱 新增

\- 🌸 支持 brush 高性能纹理扩展

#### 🌷 感谢反馈

@闰土

### [Image Filter 插件](/plugin/official/image-filter/index.md)

> v1.0.0 发布正式版

### [Linker 插件](/plugin/official/linker/index.md)

> v1.0.0 发布正式版

### [Transition 插件](/plugin/official/transition/index.md)

> v1.0.0 发布正式版

### [Stroke Sides 插件](/plugin/official/stroke-sides/index.md)

> v1.0.0 发布正式版

## 2026 / 05 / 26

> **v1.1.0**

### [Image Lighter 插件](/plugin/official/image-lighter/index.md)

> v1.1.0

#### 🌱 新增

\- 🌸 支持 brush 纹理扩展

#### 🌷 感谢反馈

@闰土

## 2026 / 04 / 20

> **v1.0.9**

### [Stroke Sides 插件](/plugin/official/stroke-sides/index.md)

> v1.0.0-beta

依赖于 LeaferJS [ v2.1.0](https://www.leaferjs.com/ui/update/#_2026-04-20)

#### 🌱 上线首个版本

#### 🌷 [申请开发插件](https://www.pxgrow.com/docs/plugin/apply.html)

@小志

### [Path Editor 插件](/plugin/official/path-editor/index.md)

> v1.0.9

###### 🪲 修复

\- 🌸 dataType采用node时，创建节点后控制台会报错的问题

\- 🌸 创建节点过程中，删除节点后再创建节点可能会不符合预期的问题

#### 🌷 感谢反馈

@闰土 @小志

## 2026 / 04 / 03

### [Path Editor 插件](/plugin/official/path-editor/index.md)

> v1.0.8

依赖于 LeaferJS [ v2.0.7](https://www.leaferjs.com/ui/update/#_2026-04-03)

#### 🌱 新增

\- 🌸 支持曲线与折线、折线与曲线之间的圆角。

\- 🌸 支持给节点单独[设置圆角](/plugin/official/path-editor/index.md#单独设置节点圆角)

#### 🌷 感谢反馈

@闰土

### [Image Lighter 插件](/plugin/official/image-lighter/index.md)

> v1.0.9

#### 🪲 修复

\- 🌸 优化透明SVG切片图的显示问题

#### 🌷 感谢反馈

@闰土

## 2026 / 03 / 30

### [Transition 插件](/plugin/official/transition/index.md)

> v1.0.0-beta

#### 🌱 上线首个版本

\- 🌸 轻松实现渐变颜色过渡动画

### [Linker 插件](/plugin/official/linker/index.md)

> v1.0.0-beta.4

#### 🌱 新增

\- 🌸 自动创建连接点支持 [多种模式](/plugin/official/linker/index.md#关键方法)，可控制只吸附4个方向、8个方向、任意点

\- 🌸 拖拽创建连线元素 [示例](/plugin/official/linker/index.md#拖拽创建连线元素)

\- 🌸 带流动动画的连线元素 [示例](/plugin/official/linker/index.md#带流动动画的连线元素)

#### 🪲 修复

\- 通过方法自动创建连接点，方向不准确的问题

#### 🌷 感谢反馈

@CML

## 更早的更新

2026年03月30日之前的更新分布在每个插件包内（插件包内有独立的更新日志）。
