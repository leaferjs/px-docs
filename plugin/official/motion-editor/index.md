...
本插件不发布于公开 NPM 仓库，通过本地 `.tgz` 文件安装使用，需 [获取插件](https://www.pxgrow.com/plugin/view/?id=10024) 授权后才能使用。

### 第一步：获取插件包

购买后，你将获得一个名为 `pxgrow-motion-editor-1.0.0-beta.tgz` 的安装包。
...
```sh [npm]
npm install ./pxgrow/pxgrow-motion-editor-1.0.0-beta.tgz
```

```sh [pnpm]
pnpm add ./pxgrow/pxgrow-motion-editor-1.0.0-beta.tgz
```

```sh [yarn]
yarn add ./pxgrow/pxgrow-motion-editor-1.0.0-beta.tgz
```

```sh [bun]
bun add ./pxgrow/pxgrow-motion-editor-1.0.0-beta.tgz
```
...
`"@pxgrow/motion-editor": "file:pxgrow/pxgrow-motion-editor-1.0.0-beta.tgz"`

---

或通过 script 标签引入，使用全局变量 PxGrow.motionEditor 访问插件内部功能。

需解压 `pxgrow-motion-editor-1.0.0-beta.tgz` 文件，复制 `package/dist/motion-editor.js` 使用。

::: code-group

```html [web]
<script src="/lib/pxgrow/motion-editor.js"></script>
<script>
  const { MotionEditTool } = PxGrow.motionEditor
</script>
```
...
```ts
// #Motion Editor [编辑运动元素位置]
import { Group, App, Line, Path } from 'leafer-ui'
import '@leafer-in/editor' // 导入动画插件
...
const group = new Group({
    ...
    editable: true,
    hitChildren: false,
    editOuter: 'MotionEditTool' // 设置运动编辑工具 // [!code hl]
})
