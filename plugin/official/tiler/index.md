...
图片平铺工具，支持双击元素或调用 `editor.openInnerEditor()` 进入平铺编辑模式。

::: tip 继承
TileEditor &nbsp;>&nbsp; [InnerEditor](https://www.leaferjs.com/ui/plugin/in/editor/InnerEditor.html)
:::
...
本插件不发布于公开 NPM 仓库，通过本地 `.tgz` 文件安装使用，需 [获取插件](https://www.pxgrow.com/plugin/view/?id=10001) 授权后才能使用。
...
```html [web]
<script src="/lib/pxgrow/tiler.js"></script>
<script>
  const { TileEditor } = PxGrow.tiler
</script>
```
...
### innerTransformTool: [`TransformTool`](./TransformTool.md)

元素编辑框的变换操作工具，支持手动移动、旋转、调整大小。

### imageTransformTool: [`TransformTool`](./TransformTool.md)

图片编辑框的变换操作工具，支持手动移动、旋转、调整大小。

## 显示属性

### innerEditBox: [`EditBox`](https://www.leaferjs.com/ui/plugin/in/editor/EditBox.html)
...
### userConfig: [`ITileEditorConfig`](./config/basic.md)

用户的配置。

### mergeConfig: [`ITileEditorConfig`](./config/basic.md)

实际使用的编辑器配置，实时合并编辑器的默认 config 与 userConfig，频繁访问会有性能开销。

### mergedConfig: [`ITileEditorConfig`](./config/basic.md)
...
const app = new App({
    view: window, editor: {
        TileEditor: { // 配置平铺编辑器
            pointColor: '#0d99ff', // 平铺控制点颜色 // [!code hl]
        }
    }
})
...
const app = new App({
    view: window, editor: {
        TileEditor: { // 配置平铺编辑器
            imageMask: 'rgba(255, 0, 0, 0.5)' // 遮罩层颜色，默认为 rgba(0,0,0,0.5)   // [!code hl]
        }
    }
})
...
### 图片只能在有效范围内拖动、平铺（暂不支持同时旋转、倾斜图片，限制效果会有误差，后续会支持）

```ts
// #Tiler [图片只能在有效范围内拖动、平铺（暂不支持同时旋转、倾斜图片，限制效果会有误差，后续会支持）]
import { App, Rect } from 'leafer-ui'
import '@leafer-in/editor' // 导入图形编辑器插件
import '@leafer-in/viewport' // 导入视口插件 (可选)
...
const app = new App({
    view: window, editor: {
        ...
        skewable: false,
        TileEditor: { // 配置平铺编辑器
            dragLimit: true // 图片在平铺框的有限范围内拖动，默认为false  // [!code hl]
        }
    }
})
...
### 在多个 fill、stroke 中指定需要平铺的图片填充对象
